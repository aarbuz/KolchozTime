...
class AppViewModel: ObservableObject {
    ...
    @Published var accentColor: Color = .blue {
        didSet {
            if let hex = accentColor.toHex() {
                UserDefaults.standard.set(hex, forKey: "AppAccentColor")
            }
        }
    }
    ...
    func getShiftTypeStats() -> [ShiftType: Double] {
        ...
        for shift in shiftsForCurrentMonth() {
    ...
    func loadShifts() {
        ...
        if let hex = UserDefaults.standard.string(forKey: "AppAccentColor") { accentColor = Color(hex: hex) }
    }
}
